...
\noindent {\bf Single Measurements Table} In the interest of
...
\yannisp{This is one way, where each tuple is an independent outcome. In the more general way, the distribution over functions approach captures the dependency of the tuples. Another choice that troubled me, is whether the S, X, Y, T are drawn Vs they are given. Eg, it is known how many (and which) sensors we have. Let's discuss on Thursday.}
In
